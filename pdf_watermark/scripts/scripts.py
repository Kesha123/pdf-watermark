...
def apply_waterwark(first_page: str, last_page: str, output_file: str, gs_script: str, file_name: str) -> str:
    return f"gs -dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -dAutoRotatePages=/None -dFirstPage={first_page} -dLastPage={last_page} -sOutputFile={output_file} {gs_script} {file_name}"

def concat_files(output_file: str) -> str:
    return f"gs -q -dNOPAUSE -sDEVICE=pdfwrite -dAutoRotatePages=/None -sOUTPUTFILE={output_file} -dBATCH "